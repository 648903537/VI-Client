...
namespace com.amtec.action
{
    public class GetNumbersOfSingleBoards
    {
        ...
        public List<MdataGetPartData> GetNumbersOfSingleBoardsResultCall(String partNumber)
        {
            ...
            string errorMsg = UtilityFunction.GetZHSErrorString(result, init, sessionContext);
            LogHelper.Info("end api mdataGetPartData (result code = " + result + ")");
            if (result != 0)
            {
                view.errorHandler(2, init.lang.ERROR_API_CALL_ERROR + " mdataGetPartData " + result+"("+errorMsg+")", "");
                return null;
            }
